...
export class LaunchConfigurator {
    ...
    async makeLaunchFile(): Promise<boolean> {
        ...
        do {
            ...
            debugConfig.name = selection === ""
                ? "Launch_template"
                : `(debugger-metaware) ${parse(execFile).base} Launch`;
    ...
    async checkLaunchConfig(): Promise<void> {
        if (!await this.isThereDebugConfig()) {
            const yes = "Yes";
            const no = "No";
            const selection = await vscode.window.showInformationMessage("Unable to identify MetaWare C++ launch configuration in your launch.json file.\
        Would you like to create a debug launch configuration now?", yes, no);
 
            if (selection === yes) {
                await vscode.commands.executeCommand("MetaWare.launchConfigurator.generateLaunchJson");
                return;
            }
            if (selection === no) {
                return;
    ...
    async checkDebugger(): Promise<void> {
    ...
    private isDebuggerInPath(): boolean {
        //TBD 
        // if (process.env.PATH) {
        //     const path = ???;
